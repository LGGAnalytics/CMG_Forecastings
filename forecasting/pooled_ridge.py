...
class ForecastConfig:
    ...
    Fields are intentionally simple so they can be JSON-serialized and shown in prints.
    """
    ...
    def hash_key(self) -> str:
        # Simple hash-like key for change detection (features + alpha)
        key = {
            "features": list(self.feature_list),
            "alpha": float(self.alpha),
            "exclude_stores": list(self.exclude_stores) if self.exclude_stores is not None else [],
        }
        return json.dumps(key, sort_keys=True)

    def excluded_set(self) -> set:
        if not self.exclude_stores:
            return set()
        s: set = set()
        for v in self.exclude_stores:
            t = str(v).strip()
            s.add(t)
            s.add(t.lstrip("0"))
        return s
...
class FeatureBuilder:
    ...
    def build_feature_table(
        ...
        events: Optional[object] = None,   # DataFrame or path to Excel with Super Bowl
        adv_keep: Optional[pd.DataFrame] = None,  # columns: store_number, Advertising_keep
        cpi: Optional[object] = None,      # DataFrame (monthly or fiscal) or path to Excel
        exclude_stores: Optional[Sequence[object]] = None  # optional list; if None, will use cfg.exclude_stores
    ) -> pd.DataFrame:
        """Builds the full feature table in one go, robust to missing pieces.

        Inputs
        - base_df: period/daily table with at least: store_number, event_date, total_net_sales, total_advertising
        ...
        - cpi: None, DataFrame, or path; either monthly with Year+Jan..Dec or fiscal with ['fiscal_year','fiscal_period','CPI']
        ...
        # Optional: exclude specific stores (single source of truth inside the builder)
        ex = {str(s).strip() for s in (exclude_stores if exclude_stores is not None else self.cfg.exclude_stores or [])}
        # also include zero-stripped variants for robust matching
        ...
        if ex:
            ...
            after = len(df)
            if before != after and self.cfg.verbose:
                print(f"[FeatureBuilder] excluded stores: {len(ex)} | rows removed: {before - after}")
        ...
        if events is not None:
            if isinstance(events, str):
                ...
                ev_df = events.copy()
            ev_df["event_date"] = pd.to_datetime(ev_df["event_date"], errors="coerce").dt.normalize()
            sb_col = None
            ...
            if sb_col is not None:
                ev_df = ev_df[["event_date", sb_col]].rename(columns={sb_col: "Super Bowl"})
                ev_df["Super Bowl"] = ev_df["Super Bowl"].fillna(0).astype(int).clip(0, 1)
                df = df.merge(ev_df, on="event_date", how="left")
        # If still missing, default 0
        if "Super Bowl" not in df.columns:
            df["Super Bowl"] = 0

        # Advertising_keep flag is no longer used for gating; ignore adv_keep and do not merge flags.

        # Calendar prep
        cal = self._ensure_calendar(calendar_df)
        # Standardize potential fiscal col names to expected
        ...
        if "is_5w" not in cal.columns:
            if "weeks_in_period" in cal.columns:
                cal["is_5w"] = (pd.to_numeric(cal["weeks_in_period"], errors="coerce") == 5).astype(int)
            elif "days_in_period" in cal.columns:
                cal["is_5w"] = (pd.to_numeric(cal["days_in_period"], errors="coerce") >= 35).astype(int)
            else:
                # No way to infer; default to 0 to avoid KeyError and warn via print
                cal["is_5w"] = 0
                print("[FeatureBuilder] WARNING: calendar lacks days_in_period/weeks_in_period; is_5w defaulted to 0.")
        ...
        if not {fy_col, fp_col}.issubset(df.columns):
            ...
            cal_daily = (
                cal.assign(event_date=cal.apply(lambda r: pd.date_range(r.get("BeginningDate"), r.get("EndingDate"), freq="D") if ("BeginningDate" in cal.columns and "EndingDate" in cal.columns) else pd.to_datetime([]), axis=1))
        ...
        if cpi is not None and "CPI_m1" not in df.columns:
            ...
            # map CPI to fiscal period end month via calendar if needed
            cpi_fiscal = self._map_cpi_to_fiscal(cal, cpi_df)
            df = df.merge(cpi_fiscal[[fy_col, fp_col, "CPI"]], on=[fy_col, fp_col], how="left")
        ...
        # No advertising gating by flags; keep raw lagged advertising.
        ...
        # Drop rows missing only critical lags/target; coerce other features to numeric and fill
        before = len(out)
        req = [cfg.target]
        ...
        if numeric_feats:
            out[numeric_feats] = out[numeric_feats].apply(pd.to_numeric, errors="coerce").fillna(0.0)
        after = len(out)
        if self.cfg.verbose:
            print(f"[FeatureBuilder] final features rows: {after} (dropped {before - after})")
        return out
    ...
    def _prepare_cpi(self, cpi: object) -> pd.DataFrame:
        ...
        if "year" in df_cols_lower and any(m.lower() in df_cols_lower for m in ["jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"]):
            months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]
            long_df = df.melt("Year", months, var_name="Month", value_name="CPI")
            long_df["cal_month"] = pd.to_datetime(
                long_df["Year"].astype(int).astype(str) + "-" + long_df["Month"].map({m:i+1 for i,m in enumerate(months)}).astype(str)
        ...
        if {self.cfg.time_cols[1], self.cfg.time_cols[2], "CPI"}.issubset(df.columns):
            # The caller will handle merge on fy/fp
            return df
        # Fallback: empty
        return pd.DataFrame({"cal_month": [], "CPI": []})

    def _map_cpi_to_fiscal(self, cal: pd.DataFrame, cpi_df: pd.DataFrame) -> pd.DataFrame:
        # If CPI is monthly with cal_month, map via calendar month end; if already fiscal return as is
        fy_col, fp_col = self.cfg.time_cols[1], self.cfg.time_cols[2]
        ...
        out = cal2[[fy_col, fp_col, "cal_month"]].merge(cpi_df, on="cal_month", how="left")
    ...
    def _ensure_calendar(self, cal_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "BeginningDate" in cal.columns:
            cal["BeginningDate"] = pd.to_datetime(cal["BeginningDate"], errors="coerce")
        if "EndingDate" in cal.columns:
            cal["EndingDate"] = pd.to_datetime(cal["EndingDate"], errors="coerce")
        if "days_in_period" not in cal.columns and {"BeginningDate", "EndingDate"}.issubset(cal.columns):
    ...
    def _compute_seasonality(self, df: pd.DataFrame) -> pd.DataFrame:
        fy_col, fp_col = self.cfg.time_cols[1], self.cfg.time_cols[2]
        out = df.copy()
        # 13-period seasonality
        if "p13_sin" in out.columns:
            out = out.drop(columns=["p13_sin"], errors="ignore")
        if "p13_cos" in out.columns:
            out = out.drop(columns=["p13_cos"], errors="ignore")
        # guard against missing fp values
        fp = pd.to_numeric(out[fp_col], errors="coerce")
        out["p13_sin"] = np.sin(2 * np.pi * fp / 13.0)
        out["p13_cos"] = np.cos(2 * np.pi * fp / 13.0)
        return out

    # Removed legacy helpers: _compute_is_5w, _compute_lag, _apply_adv_gating (logic handled inline in builders)

    def _ensure_cpi_m1(self, df: pd.DataFrame, cpi_df: Optional[pd.DataFrame]) -> pd.DataFrame:
        out = df.copy()
        fy_col, fp_col = self.cfg.time_cols[1], self.cfg.time_cols[2]
        ...
        elif cpi_df is not None and {fy_col, fp_col, "CPI"}.issubset(cpi_df.columns):
            cpi = cpi_df[[fy_col, fp_col, "CPI"]].copy()
            out = out.merge(cpi, on=[fy_col, fp_col], how="left")
        else:
            out["CPI"] = np.nan
        # Forward-fill by fiscal order per store or globally
    ...
    def _ensure_super_bowl(self, df: pd.DataFrame, cal: pd.DataFrame, events_df: Optional[pd.DataFrame]) -> pd.DataFrame:
        out = df.copy()
        ...
        if col in out.columns:
            ...
            return out
        # If not present, try events_df with mapping from year to date
        # Fallback: set 0
        out[col] = 0
        return out
    ...
    def build_forecast_set(
        ...
        """Construct one next-period row per store using last observed values and next-period calendar.

        Assumes history_df already contains the necessary columns and spans at least 1 row per store.
        """
        cfg = self.cfg
        store_col, date_col, fy_col, fp_col = cfg.store_col, cfg.time_cols[0], cfg.time_cols[1], cfg.time_cols[2]
        ...
        cal = self._ensure_calendar(calendar_df)
        # Standardize calendar columns and ensure is_5w available
        cal = self._standardize_fiscal_cols(cal)
        if "is_5w" not in cal.columns:
            if "weeks_in_period" in cal.columns:
                cal["is_5w"] = (pd.to_numeric(cal["weeks_in_period"], errors="coerce") == 5).astype(int)
        ...
        # Merge is_5w for next period (no dependency on days_in_period here)
        next_rows = next_rows.merge(
            cal[[fy_col, fp_col, "is_5w"]].drop_duplicates(),
            on=[fy_col, fp_col], how="left"
        )
        ...
        for src_col, lag_col in [(cfg.target, "total_net_sales_m1"), ("adv", "adv_m1"), ("CPI", "CPI_m1")]:
            if lag_col in last.columns:
                next_rows[lag_col] = last[lag_col].values
            elif src_col in last.columns:
                next_rows[lag_col] = pd.to_numeric(last[src_col], errors="coerce").values
            else:
                next_rows[lag_col] = 0.0

        # No advertising gating for forecast; keep adv_m1 as-is (filled numeric)
        next_rows["adv_m1"] = pd.to_numeric(next_rows["adv_m1"], errors="coerce").fillna(0.0)

        # event_date for next period can be the beginning of the fiscal period from calendar (optional)
        # If BeginningDate exists for the matching fy/fp, attach it
        if {"BeginningDate", fy_col, fp_col}.issubset(cal.columns):
            next_rows = next_rows.merge(
                cal[[fy_col, fp_col, "BeginningDate"]].rename(columns={"BeginningDate": date_col}),
                on=[fy_col, fp_col],
                how="left",
            )
        ...
        # Some features may not be available (e.g., CPI_m1), ensure presence
        for f in cfg.feature_list:
            if f not in next_rows.columns:
                next_rows[f] = 0.0
...
class PooledRidgeForecaster:
    ...
    - train(): fits on pooled stores with ≥ min_history_train; reports holdout WAPE using last k periods for stores ≥ min_history_eval
    - predict(): builds dummy-aligned design and returns forecasts with only the requested key columns
    """
    ...
    def train(self, df_features: pd.DataFrame) -> Dict[str, float]:
        ...
        if y_valid is not None and len(y_valid) > 0:
            ...
            wape = float(np.sum(np.abs(y_valid.values - preds)) / denom) if denom > 0 else float("nan")
            metrics["wape_holdout"] = wape
            if self.cfg.verbose:
                print(f"[Train] holdout WAPE: {wape:.4f} | rows_eval={len(df_valid)}")
        else:
            if self.cfg.verbose:
        ...
        if self.cfg.verbose:
            print(
                f"[Train] stores_train={int(metrics['stores_train'])} | stores_eval={int(metrics['stores_eval'])} | "
                f"rows_train={int(metrics['rows_train'])} | rows_eval={int(metrics['rows_eval'])} | alpha={cfg.alpha}"
            )
        return metrics
        ...
        for h in range(1, int(H) + 1):
            ...
            for col in ["adv_m1", "CPI_m1"]:
                if col in next_hist.columns:
                    next_hist[col] = pd.to_numeric(next_hist[col], errors="coerce").fillna(0.0)
    ...
    def needs_retrain(self, new_feature_list: Optional[Sequence[str]] = None, new_alpha: Optional[float] = None) -> bool:
        """Simple check for feature/alpha drift that should trigger retraining."""
        features = list(new_feature_list) if new_feature_list is not None else list(self.cfg.feature_list)
        alpha = float(new_alpha) if new_alpha is not None else float(self.cfg.alpha)
        exclude = list(self.cfg.exclude_stores) if self.cfg.exclude_stores is not None else []
        proposed = json.dumps({"features": features, "alpha": alpha, "exclude_stores": exclude}, sort_keys=True)
